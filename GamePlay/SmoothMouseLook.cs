...
//	[HideInInspector]
	...
	void Update(){
		...
		if(Time.timeScale > 0.0f && Time.smoothDeltaTime > 0.0f){//allow pausing by setting timescale to 0
			//var rotationChanges;
			if (InputComponent.moveX == 0)
			{
				_aimbot.timeToAim = 0.3f;

			}
            else
            {
				_aimbot.timeToAim = 1.2f;

			}
			rotationChanges = _aimbot.TrackTarget();
			//Hide the cursor
			ControlFreak2.CFCursor.lockState = CursorLockMode.Locked;
			ControlFreak2.CFCursor.visible = false;
			//vec = rotationChanges.TurnAddition;
			horizontalDelta = rotationX;//old rotationX
			rotationX += rotationChanges.TurnAddition.y+ InputComponent.lookX;//lower sensitivity at slower time settings
